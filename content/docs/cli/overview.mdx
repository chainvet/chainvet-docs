...
| --- | --- | --- | --- |
| `--mode` | `-m` | `static` · `symbolic` · `fuzzing` · `hybrid` (default `hybrid`) | Analysis engine(s). See [Engines](/docs/concepts/engines). |
| `--format` | `-f` | `pretty` · `json` · `md` · `html` · `pdf` (default `pretty`) | Output format. See [Output formats](/docs/cli/output-formats). |
...
Plus the [Hybrid tuning flags](/docs/cli/hybrid-tuning) (`--epochs`,
`--fuzz-time-ms`, `--se-depth`, `--seed`, and more) that shape the budget of
fuzzing/symbolic/hybrid runs.

See [Filtering findings](/docs/cli/filtering) for how the floor and exact filters
interact.
