...
exposes scanning over HTTP and powers the [Web UI](/integrations/web-ui).
...
<Callout title="Prefer the launcher">
  The [Web UI](/integrations/web-ui) ships a `serve.py` launcher that starts
...
[audit report](/cli/audit-reports) the CLI produces (the two front ends share
